...
final readonly class InertiaService implements IInertiaService
{
    ...
    public function addToastToCurrentRequest(string $type, string $message): void
    {
        ...
        $previouslySharedToastsOfType = $this->_inertia->getShared("toasts.$type", []);

        if (is_callable($previouslySharedToastsOfType)) {
            $previouslySharedToastsOfType = $previouslySharedToastsOfType();
        }

        if (is_iterable($previouslySharedToasts) && is_iterable($previouslySharedToastsOfType)) {
            $this->_inertia->share('toasts', [
                ...$previouslySharedToasts,
                $type => [
                    ...$previouslySharedToastsOfType,
                    $message
                ]
            ]);
        } else {
            throw new RuntimeException("Misconfigured shared Inertia data for toasts of type [$type].");
        }
    }
}
