...
type CreationRequestData = Omit<CreationForm, "parent"> & { parentInstitutionId: string | null };

export default function Create({ auth } : PageProps) {
    const { data, setData, post, processing, errors, transform } = useForm<CreationForm>({
        name: "",
        website: "",
        picture: null,
        parent: null
    ...
    }) as CreationRequestData as any);

    const [parentQuery, setParentQuery] = useState("");
    const [filteredParents, setFilteredParents] = useState<InstitutionViewModel[]>([]);

    const updateParents = useDebouncedCallback((searchQuery: string) => {
        fetch(route("api.institutions.index", {
            search: searchQuery
        }), {
            headers: {
            ...
            if (response.ok) {
                response.json().then((body: { institutions: PaginatedCollection<InstitutionViewModel>}) => {
    ...
    const [pictureInputPreview, setPictureInputPreview] = useState<string|null>(null)
                    ...
                    <div className="grid grid-cols-1 gap-x-8 gap-y-8 md:grid-cols-3">
                        <div className="px-4 sm:px-0">
                            ...
                            <p className="mt-1 text-sm leading-6 text-gray-600">
                                Information about this institution that will be publicly accessible to all users.
                            </p>
                        </div>
                        ...
                        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl md:col-span-2">
                            <div className="px-4 py-6 sm:p-8">
                                <div className="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                    ...
                                    {/* Parent */}
                                    <div className="sm:col-span-4">
                                        <Combobox as="div" value={data.parent} onChange={value => setData("parent", value)} onClose={() => setParentQuery('')}>
                                            <Label className="block text-sm font-medium leading-6 text-gray-900">Parent institution</Label>
                                            <div className="relative mt-2">
                                                <ComboboxInput
                                                    className="w-full rounded-md border-0 bg-white py-1.5 pl-3 pr-10 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                                                    onChange={event => setParentQuery(event.target.value)}
                                                    onBlur={() => setParentQuery('')}
                                                    placeholder="The institution to which this entity is subordinated"
                                                    displayValue={(institution: InstitutionViewModel) => institution?.name ?? ""}
                                                />
                                                    ...
                                                    <ComboboxOptions className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                                                        {filteredParents.map(parent => (
                                                            ...
                                                            >
                                                                {({ focus, selected }) => (
                                                                    <>
                                                                        <span className={combineClassNames('block truncate', selected ? 'font-semibold': '')}>{parent.name}</span>
                                    ...
                                    <div className="col-span-full">
                                        ...
                                        <input
                                            ...
                                            className="hidden"
                                            ref={pictureInput}
                                            onChange={e => updatePictureInputValue(e.target.files?.item(0) ?? null)}
                                        />

                                        <div className="mt-2 flex items-center gap-x-3">
                                            {pictureInputPreview
                                                ? <img src={pictureInputPreview} className="size-12 rounded-full border"
                                                       alt="Upload preview"/>
                                                : <div
                                            ...
                                            {!!pictureInputPreview?.length &&
                                                <SecondaryButton type="button"
                                                                 onClick={() => updatePictureInputValue(null)}>
                                                    Clear
                                                </SecondaryButton>
                                            }
                                            <PrimaryButton type="button" onClick={() => pictureInput.current?.click()}>
                                                Change
                                            </PrimaryButton>
                                        </div>
                                    </div>
