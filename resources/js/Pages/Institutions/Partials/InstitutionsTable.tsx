...
export default function InstitutionsTable({ institutions } : { institutions: PaginatedCollection<InstitutionViewModel>}) {
    const [filteredInstitutions, setFilteredInstitutions] = useState(institutions.data);

    const directory = useMemo(
        () => filteredInstitutions.reduce((accumulator: { [letter: string]: InstitutionViewModel[]}, institution) => {
            const firstLetter = institution.name.trim().at(0)?.toUpperCase() ?? "#";
                ...
                <nav className="h-full overflow-y-auto" aria-label="Directory">
                    {Object.keys(directory).map((letter) => (
                        <div key={letter} className="relative">
                            <div
                                className="sticky top-0 z-10 border-y border-b-gray-200 border-t-gray-100 bg-gray-50 px-3 py-1.5 text-sm font-semibold leading-6 text-gray-900">
                                <h3>{letter}</h3>
                            </div>
                            <ul role="list" className="divide-y divide-gray-100">
                                {directory[letter].map((institution) => (
                                    <li key={institution.id} className="flex gap-x-4 px-3 py-5">
                                        {institution.pictureUri
                                            ?
                                            <img className="size-12 flex-none rounded-full bg-gray-50 border shadow"
