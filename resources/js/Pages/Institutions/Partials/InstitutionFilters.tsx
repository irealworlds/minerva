...
export default function InstitutionFilters({ filters }: InstitutionFiltersProps) {
    const {data, setData, get, errors, isDirty, processing, reset, transform} = useForm<FiltersForm>({
        search: ""
    });

    transform(formData => {
        const filterParams: {
            [p in keyof typeof formData]?: string
        } = {};
            ...
            <form onSubmit={updateSearch} className="w-full max-w-xs">
                <div className="w-full">
                    ...
                    <div className="mt-4 flex items-center gap-4 flex-wrap">
                        {isDirty &&
                            <SecondaryButton className="grow justify-center" type="button" onClick={() => reset()}
                                             disabled={processing}>
                                Clear
                            </SecondaryButton>}
                        <PrimaryButton className="grow justify-center" type="submit"
                                       disabled={processing}>
                            Update results
                        </PrimaryButton>
                    </div>
